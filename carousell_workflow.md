...
比價規則：**售價 ≤ 新品 × 30%** 或 **售價 ≤ 二手行情 × 70%**，符合任一就算好貨。$3,000 以上未過門檻但 ≤85% 的進殺價保留清單。

行情數字**必須用 subagent web search 查**，不能用猜的。

**「二手行情」必須是同款實際成交或掛賣價，不是用相似款推估的。** subagent 要：
- 直接搜該型號在 Carousell/蝦皮/露天/Yahoo 拍賣的實際 listing
- 找到 ≥3 筆同款報價才算有行情，少於 3 筆要明說「資料不足」
- 不能用「同系列其他款 $X，**估**這款也差不多」的方式推估
- 找不到同款資料時，secondhand 應該回 null，不要硬填數字

更新頻率：
- **每天至少刷新一次**：用 subagent 查所有 category 的最新價格
- **新品出了就要更新**：2026 年很多產品因為新款上市，舊款大幅折舊。搜最新款的價格，回推舊款的二手行情
- **發現新貨時驗證**：商品進入清單時，確認行情表的數字還是對的

分類頁（家具居家、精品等）不設行情——太雜。分類頁只作為 scrape 來源，不進比價。
